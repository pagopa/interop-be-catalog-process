...
object CatalogProcessErrors {
  ...
  final case class UpdateEServiceError(eServiceId: String)
      extends ComponentError("0020", s"Error while updating e-service having Id $eServiceId")
  ...
  final case class CloneDescriptorError(descriptorId: String, eServiceId: String)
      extends ComponentError("0027", s"Error while cloning descriptor $descriptorId for E-service $eServiceId")

  final case class EServiceDeletionError(eServiceId: String)
      extends ComponentError("0028", s"Error while deleting E-Service $eServiceId")

  final case class ActivateDescriptorDocumentBadRequest(descriptorId: String, eServiceId: String)
      extends ComponentError("0029", s"Error while activating descriptor $descriptorId for E-Service $eServiceId")
  ...
  final case class SuspendDescriptorDocumentBadRequest(descriptorId: String, eServiceId: String)
      extends ComponentError("0032", s"Error while suspending descriptor $descriptorId for E-Service $eServiceId")
